...
use reqwest::Client;
use std::{
    borrow::Cow, collections::HashMap, error::Error, fs, sync::Mutex as StdMutex, time::Duration,
};
...
async fn execute_task(task: Task, client: &Client) -> TaskResult {
    match task {
        ...
        Task::DiskWrite { path, data } => match tokio::fs::write(&path, data).await {
            Ok(_) => TaskResult::Stored,
            Err(e) => TaskResult::Error(e.to_string()),
        },
        Task::DiskRead { path } => match tokio::fs::read(&path).await {
